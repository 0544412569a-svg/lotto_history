...
def load_data():
    """Загрузка базы тиражей без использования вызова check(), обходящая ошибку 403"""
    try:
        # Пытаемся напрямую скачать файл из облака
        client.download_sync(remote_path=FILENAME, local_path=FILENAME)
    except Exception:
        # Если скачивание не удалось (например, файла ещё нет), создаем базовую структуру
...
with st.sidebar:
    ...
    with st.form("add_draw_form", clear_on_submit=True):
        ...
        # value=None оставляет поля пустыми, не требуя стирать 0
